...
Portfoliofy uses **Next.js 14** (App Router) and is divided into two primary contexts:
...
*   As the user types or drags-and-drops items, they are modifying this *local* Zustand state.
...
Whenever you need to build a new section that contains a list of items, you should *always* use this pattern.
...
2. Define the schema for a single item (e.g., `LanguageSchema`). It **must** extend `BaseListItemSchema` (which provides `id` and `hidden` fields).
...
export function LanguagesTab({ setProjectToDelete }: { setProjectToDelete: (id: string) => void }) {
  // Pass the exact key from ResumeDataSchema ('languages')
  const listMethods = useResumeList('languages', {
    languageName: '',
    fluencyLevel: '',
  });
      ...
      renderForm={(item, updateItem) => (
        <div className="space-y-4">
          <FormInput
            ...
            onChange={(val) => updateItem({ languageName: val })}
            placeholder="e.g. English, Spanish"
          />
          <FormInput
            id="fluencyLevel"
            label="Fluency Level"
            value={item.fluencyLevel}
            onChange={(val) => updateItem({ fluencyLevel: val })}
...
2. Create a new component in `app/[username]/_components/resume/preview/sections/` (e.g., `LanguagesSection.tsx`).
3. Open `app/[username]/_components/resume/preview/ResumePreview.tsx` and map your new section key to your new component inside the `SECTION_COMPONENTS` map.

---

*If you follow these patterns, your new feature will automatically support drag-and-drop reordering, hide/show toggling, and instant local state updates!*
